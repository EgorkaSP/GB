//ищем нужный элемент в массиве с помощью метода
...
// этот метод будет возвращать индекс
int IndexOf(int[] collection, int find) // в качестве аргументов здесь массив и какой то элемент файнд
{
    int count= collection.Length; // определяем размер массвиа через функцию длина
    int index=0;
    int position=-1; // для того чтобы, в случае указания позиции заведомо отсутствующей, понять это, иначе выведет 0. 
    while(index<count)
    {
        if(collection[index]==find)
        {
           position=index; // чтобы сохранить результат из условия, надо ввести переменную 
           break;  // чтобы остановить поиск после первого вхождения  
        }
        index++;
    }
    return position;
